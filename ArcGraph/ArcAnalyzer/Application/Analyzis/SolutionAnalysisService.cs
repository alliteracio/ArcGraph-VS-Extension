...
public class SolutionAnalysisService : ISolutionAnalyzerService
{
    public async Task<SolutionAnalysisResult> AnalyzeSolutionAsync(string solutionPath, CancellationToken cancellationToken)
    {
        ...
        {
            if (degree.ContainsKey(e.SourceId)) degree[e.SourceId]++;
            if (degree.ContainsKey(e.TargetId)) degree[e.TargetId]++;
        }
        ...
        try
        {
            clusters = KMeansClusterer.AssignClusters(nodeVectors, kClusters: recommendedK);
        }
        catch (Exception ex)
        {
