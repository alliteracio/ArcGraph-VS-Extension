...
using Node = ArcAnalyzer.Domain.GraphModel.GraphNode;
using Edge = ArcAnalyzer.Domain.GraphModel.GraphEdge;
...
public static class GraphLayoutHelper
{
    public static void ComputeLayoutObsolate(IList<Node> nodes, IList<Edge> edges, int width = 800, int height = 600, int iterations = 400, double spacingFactor = 1.25)
    {
        ...
        {
            adjacency.Add((e.SourceId, e.TargetId));
            adjacency.Add((e.TargetId, e.SourceId));
        }
        ...
        for (int iter = 0; iter < iterations; iter++)
        {
            ...
            foreach (var e in edges)
            {
                if (!nodeIndex.TryGetValue(e.SourceId, out var si)) continue;
                if (!nodeIndex.TryGetValue(e.TargetId, out var ti)) continue;
