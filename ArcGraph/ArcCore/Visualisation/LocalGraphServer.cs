...
<body>
  ...
  <script>
    ...
    function clusterColor(c) {
      ...
      return clusterPalette[(c - 1) % clusterPalette.length];
    }
    ...
    function applyFilters() {
      ...
      cy.batch(() => {
        ...
        const hideExternal = document.getElementById('cb-hide-external').checked;
        const hideVuln = document.getElementById('cb-hide-vulnerable').checked;
    ...
    function updateGraph(data) {
      ...
      for (const n of (data.nodes || [])) {
        ...
        const fill = colorForGroup(nodeData.group);
        const border = clusterColor(nodeData.cluster) || '#ffffff';
        nodeData.fillColor = fill;
        nodeData.borderColor = border;
        ...
        if (nodeData.isInCycle) classes.push('in-cycle');
      ...
      if (!cy) {
        cy = cytoscape({
          ...
          style: [
            { selector: 'node', style: {
                ...
                'text-valign':'center',
                'text-halign':'center',
                'background-color': 'data(fillColor)',          // fill = layer color
                'width': 'mapData(degree, 0, 50, 24, 80)',
                'height': 'mapData(degree, 0, 50, 24, 80)',
                'border-width': 4,
                'border-color': 'data(borderColor)',            // border = cluster color
                'font-size': 10,
                'text-wrap': 'wrap',
                'text-max-width': 80
              } },
            ...
            { selector: 'node.internal', style: { } },
            // vulnerable marker overrides border to red + thicker
            { selector: 'node.vuln', style: { 'border-color': '#d9534f', 'border-width': 8 } },
            // in-cycle overrides border to yellow + thicker
        ...
        document.getElementById('cb-hide-external').addEventListener('change', () => applyFilters());
        document.getElementById('cb-hide-vulnerable').addEventListener('change', () => applyFilters());
      } else {
        cy.batch(() => {
          cy.elements().remove();
          cy.add(elements);
    ...
    function setupSse() {
      try {
        ...
        setTimeout(() => {
          if (!connected) startPolling(2000);
        }, 800);
      } catch (ex) {
        startPolling(2000);
      }
